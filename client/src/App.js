...
const PRI_COLOR = {
  URGENT: '#E24B4A', HIGH: '#BA7517', MEDIUM: '#378ADD', LOW: '#639922',
};

const HOTEL_COLORS = [
  '#7F77DD','#1D9E75','#378ADD','#BA7517','#E24B4A','#D4537E','#639922',
];

const CAT_ICON = {
  MAINTENANCE:'🔧', GUEST:'🏨', RESERVATIONS:'📅',
  VENDOR:'📦', STAFF:'👥', ADMIN:'📋', OTHER:'·',
};
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', gap: 4, marginBottom: 6, flexWrap: 'wrap' }} onClick={onToggle}>
        <span style={{ fontSize: 11, padding: '2px 7px', borderRadius: 4, background: pc + '22', color: pc, fontWeight: 700 }}>
          {em.priority}
        </span>
        <span style={{ fontSize: 11, padding: '2px 7px', borderRadius: 4, background: hc + '18', color: hc, fontWeight: 600 }}>
          {em.hotel || 'Unknown'}
        </span>
        <span style={{ fontSize: 11, padding: '2px 7px', borderRadius: 4, background: '#f3f4f6', color: '#6b7280' }}>
          {CAT_ICON[em.category] || '·'} {em.category}
        </span>
        {em.requires_response && (
          <span style={{ fontSize: 11, padding: '2px 7px', borderRadius: 4, background: '#eff6ff', color: '#3b82f6', fontWeight: 500 }}>
            ↩ Reply
          </span>
        )}
      </div>
      ...
      <div style={{ display: 'flex', gap: 6, marginTop: 8 }}>
        {prevLabel && (
          <button
            ...
            style={{ fontSize: 11, padding: '3px 10px', borderRadius: 6, border: '1px solid #e5e7eb', background: '#f9fafb', cursor: 'pointer', color: '#6b7280' }}
          >
            {prevLabel}
          </button>
        )}
        {nextLabel && (
          <button
            ...
            style={{
              ...
              border: nextStatus === 'resolved' ? '1px solid #1D9E75' : '1px solid #e5e7eb',
              background: nextStatus === 'resolved' ? '#f0fdf4' : '#fff',
              color: nextStatus === 'resolved' ? '#1D9E75' : '#374151',
            }}
          >
            {nextLabel}
          </button>
        ...
        {em.status === 'resolved' && (
          <button
            ...
            style={{ fontSize: 11, padding: '3px 10px', borderRadius: 6, border: '1px solid #fecaca', background: '#fef2f2', cursor: 'pointer', color: '#E24B4A' }}
          >
            Delete
          </button>
        )}
...
export default function App() {
  ...
  async function saveHotels(list) {
    await fetch(`${API}/api/gmail/hotels`, {
      method: 'PUT', credentials: 'include',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ hotels: list }),
    });
    setHotels(list);
  ...
  async function updateStatus(id, status) {
    await fetch(`${API}/api/gmail/emails/${id}/status`, {
        ...
        {['All', ...allHotels].map(h => {
          ...
          return (
            <button key={h} onClick={() => setFilter(h)} style={{
              ...
              border: `1px solid ${active ? hc : '#e5e7eb'}`,
              background: active ? hc + '18' : '#fff',
              color: active ? hc : '#6b7280',
              ...
              >
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingBottom: 10, borderBottom: `2px solid ${col.color}`, marginBottom: 12 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <span style={{ fontSize: 11, padding: '1px 8px', borderRadius: 999, background: col.color + '22', color: col.color, fontWeight: 600 }}>
                      {cards.length}
                    </span>
                    {col.key === 'new' && (
                      <button
                        ...
                        style={{
                          ...
                          border: `1px solid ${sortNew ? '#6b7280' : '#e5e7eb'}`,
                          background: sortNew ? '#6b7280' : '#fff',
                          color: sortNew ? '#fff' : '#6b7280',
                        }}
                      >
                    ...
                    {col.key === 'resolved' && cards.length > 0 && (
                      <button
                        ...
                        style={{ fontSize: 11, padding: '2px 8px', borderRadius: 6, border: '1px solid #fecaca', background: '#fef2f2', color: '#E24B4A', cursor: 'pointer' }}
                      >
                        Clear all
                      </button>
                    )}
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
          <div style={{ background: '#fff', borderRadius: 12, padding: 24, width: 420 }}>
            ...
            <p style={{ fontSize: 13, color: '#6b7280', margin: '0 0 12px' }}>
              One hotel name per line. Claude uses these to classify which property each email belongs to.
            </p>
            <textarea
              rows={8}
              value={hotelInput}
            ...
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 12 }}>
              <button onClick={() => setShowSettings(false)} style={{ fontSize: 13, padding: '6px 14px', borderRadius: 8, border: '1px solid #e5e7eb', background: '#fff', cursor: 'pointer' }}>
                Cancel
              </button>
              <button
                onClick={() => saveHotels(hotelInput.split('\n').map(h => h.trim()).filter(Boolean))}
                style={{ fontSize: 13, padding: '6px 14px', borderRadius: 8, border: 'none', background: '#111', color: '#fff', cursor: 'pointer' }}
              >
                Save
              </button>
            </div>
